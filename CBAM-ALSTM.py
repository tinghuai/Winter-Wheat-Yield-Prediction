...
class CBAM_ALSTM(nn.Module):
    def __init__(self):
        super(CBAM_ALSTM, self).__init__()
        ...
        # 输入的数据的时间维度为8，个例数为1，经过卷积后lstm_output : [batch_size=1, n_step=6, n_hidden =32],
    ...
    def attention_net(self, lstm_output, final_state):  # final_state=hn 为最后一层隐含层所有神经元的输出值
        ...
        # [batch_size=1, n_hidden=32, n_layer(=1)]
        attn_weights = torch.bmm(lstm_output, hidden).squeeze(2)
        # 对lstm_output和转变顺序后的hidden执行矩阵乘法[1,6,32]乘以[1,32,1]=[1,6,1] attn_weights : [1,6]
