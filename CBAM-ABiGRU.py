...
class ChannelAttentionModule(nn.Module):
    def __init__(self, channel=6, reduction=3):
...
class SpatialAttentionModule(nn.Module):
    def __init__(self):
        ...
        self.conv1d = nn.Conv1d(in_channels=2, out_channels=1, kernel_size=3, stride=1, padding=1)
        # self.act=SiLU()
        self.sigmoid = nn.Sigmoid()
...
class CBAM_ABiGRU(nn.Module):
    def __init__(self):
        super(CBAM_ABiGRU, self).__init__()
        ...
        self.lstm = nn.GRU(input_size=32, hidden_size=32,
                            num_layers=1, batch_first=True,bidirectional=True)
        self.out = nn.Linear(64, 1)
        # 输入的数据的时间维度为8，个例数为1，经过卷积后lstm_output : [batch_size=1, n_step=6, n_hidden =64],
        # hn的shape为torch.Size([1, 6, 32])
    ...
    def attention_net(self, lstm_output, final_state):  # final_state=hn 为最后一层隐含层所有神经元的输出值
        ...
        # [batch_size=1, n_hidden=64, n_layer(=1)]
        attn_weights = torch.bmm(lstm_output, hidden).squeeze(2)
        # 对lstm_output和转变顺序后的hidden执行矩阵乘法[1,6,64]乘以[1,64,1]=[1,6,1] attn_weights : [1,6]
        ...
        # [1,32,6]乘[1,6,1]->[1,32,1]
        return context, soft_attn_weights

    def forward(self, X):
        input = X  # X: [1, 8, 10]
        # 其中X为每次输入的数据，在训练中采用batch方法，每一组数据有18个，每个时次的特征数为10，共有8个时次，则X的shape为torch.Size([18, 8, 10])
        # input = input.transpose(0, 1)
        input = input.permute(0, 2, 1)
        #  input [1, 10, 8]
        input = self.conv(input)  # [1,32,6]
        input = input.permute(0, 2, 1)  # [1,6,32]
        input= self.channel_attention(input) * input
        input = self.spatial_attention(input) * input
        output,finalHiddenState = self.lstm(input,None)
        # LSTM计算 [1,6,64]
        # output = output.transpose(0, 1)  #[1,6,32]
        attn_output, attention = self.attention_net(output, finalHiddenState)
        # 进行attention机制的计算，输出的为 torch.Size([1,32]) 的二维矩阵
