...
class CNN(nn.Module):
    def __init__(self):
        ...
        self.model = nn.Sequential(
            # (-1, 5, 1164, 1255)
            # (-1, 1)
            nn.Conv2d(in_channels=5,out_channels=32,kernel_size=3,stride=1,padding=2),   #输入通道数=5（5个波段），输入图像32*32，卷积核3*3，步长=1，填充=2
            nn.MaxPool2d(16),            #
...
file_dir = "./modis_data/"
field = pd.read_csv('./field.csv', encoding='gbk')
