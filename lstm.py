...
class Net(nn.Module):
    def __init__(self):
        ...
        self.lstm = nn.LSTM(input_size=10, hidden_size=10, num_layers=1, batch_first=True)  # 一层LSTM
        self.fc = nn.Linear(in_features=10, out_features=1)  # 全连接层(线性层)hidden_size对应于LSTM的output（全连接层的in_feature）
