...
class LSTM_Attention(nn.Module):
    ...
    # 输入的数据的时间维度为8，个例数为1，lstm_output : [batch_size=1, n_step=8, n_hidden =10],
    ...
    def attention_net(self, lstm_output, final_state):   #final_state=hn 为最后一层隐含层所有神经元的输出值
        ...
        # [batch_size=1, n_hidden=10, n_layer(=1)]
        attn_weights = torch.bmm(lstm_output, hidden).squeeze(2)
        #对lstm_output和转变顺序后的hidden执行矩阵乘法[1,8,10]乘以[1,10,1]=[1,8,1] attn_weights : [1,8]
        soft_attn_weights = F.softmax(attn_weights, 1)
        #其中1为计算的维度，dim=1 时表示按列进行softmax，每一列的加和为1，dim=0时表示按行进行加和，每一行的加和为1
        # context : [batch_size, n_hidden]
    ...
    def forward(self, X):
        ...
        #对output的输出结果的第一维和第二维进行转变，即：torch.Size([8, 1, 10]) — > torch.Size([1, 8, 10])
        attn_output, attention = self.attention_net(output, finalHiddenState)
        #进行attention机制的计算，输出的为 torch.Size([1, 10]) 的二维矩阵
...
for i in range(years):   #按年进行划分，一共15年
    ...
    # t_list = pd.DataFrame(y_true_all)
    # t_list.to_csv('./t_list.csv',index=None)
    p_list = pd.DataFrame(y_pre_all)
    p_list.to_csv('./p_list.csv',index=None)
