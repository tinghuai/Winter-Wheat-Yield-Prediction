...
class Net(nn.Module):
    def __init__(self):
        ...
        # self.lstm = nn.LSTM(input_size=6, hidden_size=6, num_layers=1, batch_first=True)   #一层LSTM
        # self.fc = nn.Linear(in_features=6, out_features=1)   #全连接层(线性层)hidden_size对应于LSTM的output（全连接层的in_feature）
        self.bilstm = nn.LSTM(input_size=10, hidden_size=10, num_layers=1, batch_first=True, bidirectional=True)  # 一层LSTM
        self.fc = nn.Linear(in_features=20, out_features=1)  # 全连接层(线性层)hidden_size对应于LSTM的output（全连接层的in_feature）
        # self.fc2 = nn.Linear(in_features=32, out_features=1)  # 全连接层(线性层)hidden_size对应于LSTM的output（全连接层的in_feature）
        # self.dropout = nn.Dropout(p=0.3)
    def forward(self, x):
        # x is input, size (batch_size, seq_len, input_size)
        # x= self.dropout(x)
        x, _ = self.bilstm(x,None)
